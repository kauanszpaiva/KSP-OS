...
const fingerprintManifestPath = 'docs/deployment/REPOSITORY_SCHEMA_FINGERPRINT.json';
...
if (!fs.existsSync(fingerprintManifestPath)) {
  throw new Error(`required repository schema fingerprint manifest is missing: ${fingerprintManifestPath}`);
}

const fingerprintManifest = JSON.parse(fs.readFileSync(fingerprintManifestPath, 'utf8'));
if (fingerprintManifest.schemaVersion !== 1) {
  throw new Error('unsupported repository schema fingerprint manifest version');
}
if (!/^[a-f0-9]{64}$/.test(fingerprintManifest.activeSchemaSha256 ?? '')) {
  throw new Error('repository schema fingerprint manifest has an invalid SHA256');
}

const fingerprintFailures = [];
if (fingerprintManifest.activeMigrationCount !== activeMigrations.length) {
  fingerprintFailures.push(
    `active migration count expected ${fingerprintManifest.activeMigrationCount}, got ${activeMigrations.length}`
  );
}
if (fingerprintManifest.deferredMigrationCount !== deferredMigrations.length) {
  fingerprintFailures.push(
    `deferred migration count expected ${fingerprintManifest.deferredMigrationCount}, got ${deferredMigrations.length}`
  );
}
if (fingerprintManifest.activeSchemaSha256 !== fingerprint) {
  fingerprintFailures.push(
    `active schema fingerprint expected ${fingerprintManifest.activeSchemaSha256}, got ${fingerprint}`
  );
}
if (fingerprintFailures.length) {
  throw new Error(
    `repository schema fingerprint mismatch:\n- ${fingerprintFailures.join('\n- ')}\nIf the schema change is intentional, update the reviewed fingerprint manifest in the same PR.`
  );
}
