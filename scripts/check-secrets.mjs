...
const bad=[]; for(const f of walk('.')){const norm=f.replace(/\\/g,'/'); if(norm==='scripts/check-secrets.mjs')continue; if(!/\.(ts|tsx|js|mjs|json|md|sql|env|yml|yaml)$/.test(norm))continue; const s=fs.readFileSync(f,'utf8'); if(patterns.some(p=>p.test(s)))bad.push(f)}
