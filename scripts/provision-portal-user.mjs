#!/usr/bin/env node
/**
 * Provision a client-portal login for KSP Dominion OS.
 *
 * Creates (idempotently) a Supabase Auth user, a matching `profiles` row, the
 * client organization the person belongs to (e.g. "Bez Group"), and a
 * `client_memberships` row with a `client_role`, so the client can sign in at
 * the portal's /login and resolve an active client membership via
 * getPortalAuthContext — seeing ONLY their own client organization's published
 * data (enforced by RLS `is_portal_member`).
 *
 * This is the service-role provisioning path (bypasses RLS) — the same trust
 * level as scripts/provision-internal-user.mjs. The in-app path for real use is
 * the invitation flow (portal_invitations -> accept_portal_invitation); this
 * script is for seeding/admin bootstrap.
 *
 * Secrets are NEVER read from source. Credentials come from environment
 * variables only, and the password is never printed or persisted by this script.
 *
 * Usage (run against your OWN Supabase project — do not target Production):
 *
 *   NEXT_PUBLIC_SUPABASE_URL="https://<project>.supabase.co" \
 *   SUPABASE_SERVER_ONLY_SECRET_KEY="<privileged server key>" \
 *   PORTAL_EMAIL="everton@bezgroup.com" \
 *   PORTAL_PASSWORD="<the password>" \
 *   PORTAL_NAME="Everton" \
 *   CLIENT_ORG_NAME="Bez Group" \
 *   PORTAL_ROLE="client_owner" \
 *   node scripts/provision-portal-user.mjs
 *
 * Optional: CLIENT_ORG_LEGAL_NAME (default = CLIENT_ORG_NAME),
 * ORG_SLUG (default "ksp-dominion"), ORG_NAME (default "KSP Dominion Group").
 *
 * Uses only Node's global fetch (Node 22+) against the Supabase Auth Admin API
 * and PostgREST — no extra dependencies to install.
 */

const CLIENT_ROLES = new Set([
  'client_owner',
  'client_project_approver',
  'client_billing_contact',
  'client_collaborator',
  'client_viewer'
]);

function required(name) {
  const value = process.env[name];
  if (!value || value.trim().length === 0) {
    fail(`Missing required environment variable: ${name}`);
  }
  return value.trim();
}

function fail(message) {
  console.error(`\n  provision-portal-user: ${message}\n`);
  process.exit(1);
}

function assertAllowedUrl(url) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    fail(`NEXT_PUBLIC_SUPABASE_URL is not a valid URL: ${url}`);
  }
  if (parsed.protocol !== 'https:' || !parsed.hostname.endsWith('.supabase.co')) {
    fail('NEXT_PUBLIC_SUPABASE_URL must be an https://*.supabase.co URL.');
  }
  return parsed.origin;
}

async function readError(response) {
  const text = await response.text().catch(() => '');
  return `${response.status} ${response.statusText}${text ? ` — ${text}` : ''}`;
}

async function main() {
  const baseUrl = assertAllowedUrl(required('NEXT_PUBLIC_SUPABASE_URL'));
  const serviceKey =
    process.env.SUPABASE_SERVER_ONLY_SECRET_KEY?.trim() ||
    process.env.SUPABASE_SERVER_ONLY_SERVICE_KEY?.trim();
  if (!serviceKey) {
    fail('Set SUPABASE_SERVER_ONLY_SECRET_KEY (or legacy SUPABASE_SERVER_ONLY_SERVICE_KEY).');
  }

  const email = required('PORTAL_EMAIL').toLowerCase();
  const password = required('PORTAL_PASSWORD');
  const displayName = process.env.PORTAL_NAME?.trim() || email;
  const clientOrgName = required('CLIENT_ORG_NAME');
  const clientOrgLegalName = process.env.CLIENT_ORG_LEGAL_NAME?.trim() || clientOrgName;
  const clientRole = (process.env.PORTAL_ROLE?.trim() || 'client_owner').toLowerCase();
  const orgSlug = process.env.ORG_SLUG?.trim() || 'ksp-dominion';
  const orgName = process.env.ORG_NAME?.trim() || 'KSP Dominion Group';

  if (!CLIENT_ROLES.has(clientRole)) {
    fail(`PORTAL_ROLE "${clientRole}" is not a valid client_role. One of: ${[...CLIENT_ROLES].join(', ')}`);
  }

  const authHeaders = { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` };
  const restHeaders = { ...authHeaders, 'Content-Type': 'application/json' };

  // 1) Create the auth user (email confirmed so they can sign in immediately).
  let userId;
  const createRes = await fetch(`${baseUrl}/auth/v1/admin/users`, {
    method: 'POST',
    headers: { ...authHeaders, 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, password, email_confirm: true })
  });

  if (createRes.ok) {
    const created = await createRes.json();
    userId = created.id;
    console.log(`Created auth user for ${email}.`);
  } else if (createRes.status === 422 || createRes.status === 409) {
    userId = await findUserIdByEmail(baseUrl, authHeaders, email);
    if (!userId) fail(`Auth user for ${email} exists but could not be located.`);
    const updateRes = await fetch(`${baseUrl}/auth/v1/admin/users/${userId}`, {
      method: 'PUT',
      headers: { ...authHeaders, 'Content-Type': 'application/json' },
      body: JSON.stringify({ password, email_confirm: true })
    });
    if (!updateRes.ok) fail(`Failed to update existing user password: ${await readError(updateRes)}`);
    console.log(`Auth user for ${email} already existed — password reset.`);
  } else {
    fail(`Failed to create auth user: ${await readError(createRes)}`);
  }

  // 2) Upsert the profile (id must equal the auth user id). Portal users have a
  //    profile but NO organization_membership — that is what keeps them out of
  //    every internal table (is_internal_member returns false).
  const profileRes = await fetch(`${baseUrl}/rest/v1/profiles`, {
    method: 'POST',
    headers: { ...restHeaders, Prefer: 'resolution=merge-duplicates,return=minimal' },
    body: JSON.stringify({ id: userId, display_name: displayName, email })
  });
  if (!profileRes.ok) fail(`Failed to upsert profile: ${await readError(profileRes)}`);
  console.log('Profile row is in place.');

  // 3) Resolve the tenant organization (create it if missing).
  const orgId = await ensureOrganization(baseUrl, restHeaders, orgSlug, orgName);

  // 4) Resolve the client organization the person belongs to (create if missing).
  const clientOrgId = await ensureClientOrganization(baseUrl, restHeaders, orgId, clientOrgName, clientOrgLegalName);

  // 5) Upsert the client membership (unique on client_organization_id, profile_id, role).
  const membershipRes = await fetch(`${baseUrl}/rest/v1/client_memberships`, {
    method: 'POST',
    headers: { ...restHeaders, Prefer: 'resolution=merge-duplicates,return=minimal' },
    body: JSON.stringify({
      organization_id: orgId,
      client_organization_id: clientOrgId,
      profile_id: userId,
      role: clientRole
    })
  });
  if (!membershipRes.ok) fail(`Failed to upsert client membership: ${await readError(membershipRes)}`);

  console.log(`Client membership set: ${email} -> ${clientRole} @ ${clientOrgName}.`);
  console.log(`\nDone. ${email} can now sign in at the portal /login and will see only ${clientOrgName}'s published data.`);
}

async function findUserIdByEmail(baseUrl, authHeaders, email) {
  for (let page = 1; page <= 20; page += 1) {
    const res = await fetch(`${baseUrl}/auth/v1/admin/users?page=${page}&per_page=200`, {
      headers: authHeaders
    });
    if (!res.ok) fail(`Failed to list users: ${await readError(res)}`);
    const body = await res.json();
    const users = Array.isArray(body) ? body : body.users ?? [];
    if (users.length === 0) return null;
    const match = users.find((u) => (u.email ?? '').toLowerCase() === email);
    if (match) return match.id;
  }
  return null;
}

async function ensureOrganization(baseUrl, restHeaders, slug, name) {
  const lookup = await fetch(
    `${baseUrl}/rest/v1/organizations?slug=eq.${encodeURIComponent(slug)}&select=id`,
    { headers: restHeaders }
  );
  if (!lookup.ok) fail(`Failed to look up organization: ${await readError(lookup)}`);
  const rows = await lookup.json();
  if (rows.length > 0) return rows[0].id;

  const insert = await fetch(`${baseUrl}/rest/v1/organizations`, {
    method: 'POST',
    headers: { ...restHeaders, Prefer: 'return=representation' },
    body: JSON.stringify({ name, slug })
  });
  if (!insert.ok) fail(`Failed to create organization: ${await readError(insert)}`);
  const created = await insert.json();
  console.log(`Created organization "${name}" (${slug}).`);
  return created[0].id;
}

async function ensureClientOrganization(baseUrl, restHeaders, orgId, displayName, legalName) {
  const lookup = await fetch(
    `${baseUrl}/rest/v1/client_organizations?organization_id=eq.${orgId}&display_name=eq.${encodeURIComponent(displayName)}&select=id`,
    { headers: restHeaders }
  );
  if (!lookup.ok) fail(`Failed to look up client organization: ${await readError(lookup)}`);
  const rows = await lookup.json();
  if (rows.length > 0) return rows[0].id;

  const insert = await fetch(`${baseUrl}/rest/v1/client_organizations`, {
    method: 'POST',
    headers: { ...restHeaders, Prefer: 'return=representation' },
    body: JSON.stringify({ organization_id: orgId, legal_name: legalName, display_name: displayName, status: 'active' })
  });
  if (!insert.ok) fail(`Failed to create client organization: ${await readError(insert)}`);
  const created = await insert.json();
  console.log(`Created client organization "${displayName}".`);
  return created[0].id;
}

main().catch((error) => fail(error?.message ?? String(error)));
