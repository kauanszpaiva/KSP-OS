...
import { resolveBrowserSupabaseConfig } from './supabase-browser';
...
describe('KSP INC browser auth routing', () => {
  it('pins the public standalone hostname to production Auth even with stale preview env', () => {
    ...
    expect(resolveBrowserSupabaseConfig('ksp-os-inc.vercel.app')).toEqual({
      url: PROD_URL,
      key: PROD_KEY,
    });
  });
