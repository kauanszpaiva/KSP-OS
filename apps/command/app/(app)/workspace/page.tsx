...
import { getMembers, getTasks, type TaskView } from '../data';
import { EmptyState, PageHeader, Panel, SectionLabel } from '../_components/ui';
import { CompleteTaskForm, TaskForm, TaskStatusForm } from '../_components/mission-workspace-forms';

function TaskRow({ task }: { task: TaskView }) {
  const overdue = isOverdue(task.due_date);
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-4 py-3 transition-colors duration-fast first:border-t-0 hover:bg-surface-2">
        ...
        <div className="space-y-8">
          {blocked.length > 0 && (
            <Reveal>
              ...
              <Panel>{blocked.map((t) => <TaskRow key={t.id} task={t} />)}</Panel>
            </Reveal>
          )}
          <Reveal delay={60}>
