...
/** Create a blueprint from a form; redirects into its editor on success. */
export async function createBlueprintForm(_prev: { ok: boolean; error?: string }, form: FormData): Promise<{ ok: boolean; error?: string }> {
...
/** Update name / description / kind / project link. */
export async function updateBlueprint(input: { id: string; name?: string; description?: string; kind?: string; projectId?: string | null }): Promise<BlueprintActionResult> {
  await requireSession();
  const supabase = await getServerSupabase();
  if (!supabase) return { ok: false, error: 'Database is not configured.' };

  const parsed = updateBlueprintSchema.safeParse(input);
  if (!parsed.success) return { ok: false, error: firstIssue(parsed.error) };

  const { error } = await supabase.from('blueprints').update(parsed.data).eq('id', parsed.data.id);
  if (error) return { ok: false, error: 'Could not update the blueprint.' };
...
/** Persist the interactive flowchart canvas (nodes + edges JSON). */
export async function saveBlueprintCanvas(input: { id: string; canvas: unknown }): Promise<BlueprintActionResult> {
  await requireSession();
  const supabase = await getServerSupabase();
  if (!supabase) return { ok: false, error: 'Database is not configured.' };
...
/** Move a blueprint between draft / active / archived. */
export async function setBlueprintStatus(input: { id: string; status: string }): Promise<BlueprintActionResult> {
  await requireSession();
  const supabase = await getServerSupabase();
  if (!supabase) return { ok: false, error: 'Database is not configured.' };
  ...
  const { error } = await supabase.from('blueprints').update({ status: parsed.data.status }).eq('id', parsed.data.id);
