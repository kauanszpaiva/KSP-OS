...
function NavRow({ item, active, collapsed }: { item: NavItem; active: boolean; collapsed: boolean }) {
  if (item.status === 'planned') {
    return (
      <span
        className={cx(
          'flex items-center rounded-lg py-2 text-[13px] text-ink-4',
          collapsed ? 'justify-center px-2' : 'justify-center px-2 xl:justify-start xl:gap-3 xl:px-2.5'
        )}
        aria-disabled="true"
        title={`${item.label} — coming soon`}
      >
        <Icon name={item.icon} className="h-[18px] w-[18px] shrink-0" />
        {!collapsed && <span className="hidden flex-1 truncate xl:inline">{item.label}</span>}
        {!collapsed && <span className="hidden text-[9px] font-semibold uppercase tracking-wider text-ink-4 xl:inline">Soon</span>}
      </span>
    );
  }
  return (
    ...
    >
      ...
      <Icon name={item.icon} className="h-[18px] w-[18px] shrink-0" />
      {!collapsed && <span className="hidden truncate xl:inline">{item.label}</span>}
    </Link>
  );
}
...
export function Shell({
  ...
  return (
    <div className="flex min-h-screen w-full bg-canvas text-ink">
      ...
      {/* Tablet rail / desktop sidebar. Tablet intentionally uses icons only. */}
      <aside
        className={cx(
          'sticky top-0 hidden h-screen shrink-0 flex-col border-r border-line bg-surface md:flex',
          collapsed ? 'md:w-[64px]' : 'md:w-[64px] xl:w-[248px]'
        )}
      >
        ...
        <nav aria-label="Primary" className="flex-1 space-y-4 overflow-y-auto px-2.5 py-3 xl:space-y-5 xl:py-4">
          ...
          {filtered ? (
            <div className="space-y-0.5">
              {filtered.length === 0 ? (
                <p className="hidden px-2.5 py-2 text-[12px] text-ink-4 xl:block">No modules match.</p>
              ) : (
                filtered.map((item) => <NavRow key={item.href} item={item} active={isActive(item.href)} collapsed={collapsed} />)
              )}
            </div>
          ) : (
            groups.map((group) => (
              <div key={group.key}>
                {!collapsed && (
                  <p className="hidden px-2.5 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-4 xl:block">{group.label}</p>
                )}
                <div className="space-y-0.5">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="app-topbar sticky top-0 z-20 flex items-center gap-2 border-b border-line bg-surface/88 px-3 backdrop-blur sm:px-4 md:px-5 xl:px-6">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-1.5">
            ...
            <details className="group relative">
              <summary className="flex min-h-9 min-w-9 cursor-pointer select-none items-center justify-center rounded-full transition-transform duration-fast marker:hidden hover:scale-105 [&::-webkit-details-marker]:hidden">
                <Avatar name={user.displayName} />
              </summary>
        ...
        {/* Phone navigation. Tablets move to the persistent rail above. */}
        <nav aria-label="Primary mobile" className="mobile-bottom-nav fixed inset-x-0 bottom-0 z-20 flex border-t border-line bg-surface/96 backdrop-blur md:hidden">
          {mobilePrimary.map((item) => {
            ...
            return (
              <Link
                ...
                aria-current={active ? 'page' : undefined}
                className={cx(
                  'relative flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 px-1 py-1.5 text-[10px] font-medium transition-colors duration-fast',
          ...
          <button
            ...
            className={cx(
              'relative flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 px-1 py-1.5 text-[10px] font-medium transition-colors duration-fast',
              moreActive ? 'text-brand' : 'text-ink-3 active:text-ink'
            )}
          >
  ...
  return (
    <div className="fixed inset-0 z-40 md:hidden">
      ...
      <div
        ...
        className={cx(
          'mobile-sheet absolute inset-x-0 bottom-0 max-h-[84vh] overflow-y-auto rounded-t-2xl border-t border-line bg-surface px-4 pt-4 shadow-pop',
          closing ? 'animate-slide-out-down' : 'animate-fade-slide-up'
        )}
      >
        <div className="mb-3 flex items-center justify-between">
          <div>
            <p className="text-[14px] font-semibold text-ink">More</p>
            <p className="mt-0.5 text-[11.5px] text-ink-4">Secondary company modules</p>
          </div>
