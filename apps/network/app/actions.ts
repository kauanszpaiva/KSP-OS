...
import { isNetworkViewAsActive } from '../lib/view-as';
...
const VIEW_AS_READ_ONLY_ERROR =
  'View As is read-only. Exit View As before making changes.';
...
const INVITATION_ERRORS: Record<string, string> = {
  ...
  partner_organization_inactive:
    'This partner organization is not accepting invitations.',
  partner_membership_exists:
    'This account already has Network access for this partner.',
  invitation_context_invalid: 'This invitation context is invalid.',
  partner_invitation_scope_not_supported:
    'This invitation has a scope that Network cannot safely activate yet.',
  partner_invitation_team_scope_not_supported:
    'This invitation has a team scope that Network cannot safely activate yet.'
};
...
export async function acceptPartnerInvitation(
  ...
  if (!supabase) {
    return {
      ok: false,
      error: 'KSP Network is not configured in this environment.'
    };
  }
  if (await isNetworkViewAsActive(supabase)) {
    return { ok: false, error: VIEW_AS_READ_ONLY_ERROR };
  }
  ...
  if (userError || !userData.user) {
    return {
      ok: false,
      error: 'Sign in first, then open the invitation link again.'
    };
  }

  const parsed = acceptPartnerInvitationSchema.safeParse({
  ...
  if (error) {
    ...
    return {
      ok: false,
      error:
        INVITATION_ERRORS[key] ??
        'Could not accept this invitation. Contact KSP if this continues.'
    };
  }
...
export async function respondToAssignment(form: FormData): Promise<void> {
  const supabase = await getServerSupabase();
  if (!supabase || (await isNetworkViewAsActive(supabase))) return;
  ...
  if (
    !['accepted', 'declined', 'clarification_requested'].includes(response)
  ) {
    return;
  }
