...
import { formatCount } from './data-viz-model';

/**
 * Server-rendered value, client-side count-up.
 *
 * The first render (server and hydration) always shows the real value, so no
 * placeholder can be mistaken for data. The animation then runs in a layout
 * effect, which React flushes before the browser paints, so the operator sees the
