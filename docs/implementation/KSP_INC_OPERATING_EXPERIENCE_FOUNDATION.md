...
This foundation is stacked on `feat/ksp-click-profile-v1` (PR #110), which itself depends on PR #109. It must not be retargeted to `main` until the dependency chain has been reconciled, because the earlier branches modify operating-division, profile and shell behavior used by later redesign stages.
...
- Full repository typecheck/tests/build and Vercel previews must pass before merging the foundation into its dependency branch.
