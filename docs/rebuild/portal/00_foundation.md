...
- ~~The invitation *creation* side (an internal "invite a client contact" UI/action) was not built~~ **Built (follow-up).** The Command Clients module now has an executive-only "Invite to portal" form per client (`InviteContactForm` in `growth-forms.tsx`, wired into `clients-view.tsx`). `createPortalInvitation` (command `actions.ts`) generates a random token, stores only its sha256 as `token_hash`, inserts via the pre-existing internal-only `portal_invitations_internal` RLS policy, and returns the one-time `/invite/<token>` link to hand to the client (the raw token is never persisted). Gate is `isExecutive`, matching member-management. Validated by `createPortalInvitationSchema` (5 unit tests).
