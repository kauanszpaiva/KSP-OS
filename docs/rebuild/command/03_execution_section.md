...
## Phase V2 addition (Command-wide visual redesign) — Missions Timeline, Workspace Board/Calendar, Horizon/Team charts

Part of the multi-phase Asana/ClickUp-style visual redesign (`docs/rebuild/command/07_visual_redesign_v0_foundation.md`). Applies V0's `Timeline`/`Board`/`Calendar`/chart primitives to the rest of the Execution section.

| Task | Status | Detail |
|---|---|---|
| V2.1 Missions — Timeline | ✅ | `apps/command/app/(app)/_components/missions-view.tsx` (new) — `Cards` / `Timeline` `Segmented` toggle, extracted from the page's previously-inline card grid. Missions themselves carry no dates, so each Timeline row is rolled up from that mission's own dated milestones (`start = min(start_date ?? due_date)`, `end = max(due_date)`); a mission with zero dated milestones is excluded from the Timeline (still visible in Cards). Real duration bars render whenever a mission's rolled-up start/end differ, using V0's `start_date` migration on `mission_milestones`. **Dependency lines land here, as V0's doc predicted**: `mission_dependencies` edges render as "⛓ waits on: …" text annotations on the dependent mission's row — not a drawn elbow-connector line, the same stated v1 simplification V0 already documented for the general `TimelineDependency` type. |
| V2.2 Workspace — Board + Calendar | ✅ | `apps/command/app/(app)/_components/workspace-view.tsx` (new) — `List` / `Board` / `Calendar` toggle. Board columns are Blocked / Open / Done, a derived composite of `status` + `blocked` rather than a single field — cards reuse the exact existing `TaskStatusForm`/`CompleteTaskForm` (same governed-action principle as Decisions' Board in V1: no generic "move card" control that could bypass the real status/blocked transition rules). Calendar places tasks on their `due_date`; undated tasks are omitted from Calendar (still visible in List/Board). |
| V2.3 Horizon — Chart | ✅ | `apps/command/app/(app)/_components/horizon-view.tsx` (new) — `List` / `Chart` toggle. Chart tab: a `BarChart` of due-soon load bucketed into "This week" (0–7d) / "Next 3 weeks" (8–30d) / "Next 2 months" (31–90d), clipped to the page's active 7/30/90-day range picker, plus a `Donut` of commitments-vs-milestones split. |
| V2.4 Team — Chart | ✅ | `apps/command/app/(app)/_components/team-view.tsx` (new) — `List` / `Chart` toggle. Chart tab: a `BarChart` of open load (commitments + tasks) per person, sorted descending, tinted `warn` past the existing overload threshold; a `Donut` of total open commitments vs. total open tasks company-wide. |
| V2.5 Tests | — | No new Zod schema/mutation this phase — all four views read existing data shapes (`MissionView`, `TaskView`, `HorizonItem`, `TeamLoadView`) and reuse existing actions/forms as-is. No new unit tests needed; full suite (93 tests) still green. |
| V2.6 Docs | ✅ | This section. |

**What changed vs. the V2 plan**: nothing about scope — this is a purely additive visual increment layered on top of Missions/Workspace/Horizon/Team exactly as C3.1–C3.5 already shipped them, not a revision of those modules' underlying data or authorization rules. Schedule's own Timeline application was already done in C3.6/V0 (see above) and needed no further V2 work.
